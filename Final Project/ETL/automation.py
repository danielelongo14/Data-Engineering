...
dsn_uid = "ypf66634"        # e.g. "abc12345"
dsn_pwd = "FjfIPsvykOhADVuL"      # e.g. "7dBZ3wWt9XN6$o0J"
...
# Connect to MySQL
connection = mysql.connector.connect(user='root', password='MTAyNTQtZGFuaWVs',host='127.0.0.1',database='sales')
...
def get_latest_records(rowid):
	SQL = f"SELECT *,0 as price, " + "'" + "".join(str(datetime.datetime.now().strftime(("%Y-%m-%d %H:%M:%S")))) + "'" + f"FROM sales_data WHERE rowid > {rowid}"
